...
from tools.enhancement import build_brightness_clique_case


@cocotb.test()
async def test_brightness_clique_approx(dut):
    ...
    case = build_brightness_clique_case(
        width=8,
        height=1,
        pixels=[10, 11, 12, 40, 42, 43, 120, 121],
        brightness=20,
        threshold=3,
    )

    program_memory = Memory(dut=dut, addr_bits=8, data_bits=16, channels=program_channels, name="program")
    program = assemble_file("kernels/brightness_clique_approx.asm", symbols=case["symbols"])
    ...
    actual = []
    for descriptor_index in range(case["thread_count"]):
