...
from tools.enhancement import build_adaptive_gamma_lut_case


@cocotb.test()
async def test_adaptive_gamma_lut(dut):
    ...
    case = build_adaptive_gamma_lut_case(
        width=4,
        height=2,
        pixels=[8, 16, 24, 32, 48, 64, 96, 128],
        target_mean=0.5,
        min_gamma=0.35,
    )

    program_memory = Memory(dut=dut, addr_bits=8, data_bits=16, channels=program_channels, name="program")
    program = assemble_file("kernels/adaptive_gamma_lut.asm", symbols=case["symbols"])
    ...
    await setup(
        ...
        data=case["data"],
        threads=len(case["expected_pixels"]),
    )
    ...
    actual = data_memory.memory[: len(case["expected_pixels"])]
    assert actual == case["expected_pixels"]
    assert case["metadata"]["gamma"] <= 1.0
