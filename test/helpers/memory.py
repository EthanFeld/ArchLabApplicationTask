...
class Memory:
    def __init__(self, dut, addr_bits, data_bits, channels, name):
        ...
        self.prev_read_valid = [0] * channels
        self.prev_write_valid = [0] * channels
    ...
    def run(self):
        ...
        for i in range(self.channels):
            if mem_read_valid[i] == 1:
                ...
                mem_read_ready[i] = 1
                if self.prev_read_valid[i] == 0:
                    self.read_transactions += 1
            else:
                mem_read_ready[i] = 0
            self.prev_read_valid[i] = mem_read_valid[i]
        ...
        if self.name != "program":
            ...
            for i in range(self.channels):
                if mem_write_valid[i] == 1:
                    ...
                    mem_write_ready[i] = 1
                    if self.prev_write_valid[i] == 0:
                        self.write_transactions += 1
                else:
                    mem_write_ready[i] = 0
                self.prev_write_valid[i] = mem_write_valid[i]
