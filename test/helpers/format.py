...
def format_cycle(dut, cycle_id: int, thread_id: Optional[int] = None):
    ...
    for core in dut.cores:
        ...
        total_threads = to_int(dut.thread_count.value)
        core_index = to_int(core.i.value)
        threads_per_block = to_int(dut.THREADS_PER_BLOCK.value)
        if total_threads <= core_index * threads_per_block:
            continue
        ...
        for thread in core.core_instance.threads:
            if to_int(thread.i.value) < to_int(core.core_instance.thread_count.value): # if enabled
                block_idx = to_int(core.core_instance.block_id.value)
                block_dim = to_int(core.core_instance.THREADS_PER_BLOCK.value)
                thread_idx = to_int(thread.register_instance.THREAD_ID.value)
                idx = block_idx * block_dim + thread_idx

                rs = to_int(thread.register_instance.rs.value)
                rt = to_int(thread.register_instance.rt.value)

                reg_input_mux = to_int(core.core_instance.decoded_reg_input_mux.value)
                alu_out = to_int(thread.alu_instance.alu_out.value)
                lsu_out = to_int(thread.lsu_instance.lsu_out.value)
                constant = to_int(core.core_instance.decoded_immediate.value)

                if (thread_id is None or thread_id == idx):
                    logger.debug(f"\n+-------- Thread {idx} --------+")

                    logger.debug("PC:", to_int(core.core_instance.current_pc.value))
